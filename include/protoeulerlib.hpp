...
decon_consttoPrim2 := SIMTISum(ASIMTKernelFlag(ASIMTGridDimX(QuoInt(i2.range,256)+1)), b, QuoInt(i2.range,256)+1, SIMTISum(ASIMTBlockDimX(256), i2, 256, COND(lt(add(mul(b,256),i2), Product(deconvolve_dims)),BB(Scat(fStack(fAdd(4*Product(deconvolve_dims),1,add(mul(b,256),i2)), fAdd(4*Product(deconvolve_dims),1,add(add(mul(b,256),i2), Product(deconvolve_dims))), fAdd(4*Product(deconvolve_dims),1,add(add(mul(b,256),i2), 2*Product(deconvolve_dims))), fAdd(4*Product(deconvolve_dims),1,add(add(mul(b,256),i2), 3 * Product(deconvolve_dims))))) * VStack(sf1_c2p2, rv_c2p2 * sf2_c2p2) * IterVStack(j2, j2.range, BB(_decon)).unroll()), O(4 * Product(deconvolve_dims), 4 * Product(input_dims)))));
